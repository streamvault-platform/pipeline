...
import io.streamvault.pipeline.consumer.{MediaUploadedConsumer, WatchSyncConsumer}
...
object Main extends ZIOAppDefault:
  ...
  override def run: ZIO[Any, Any, Any] =
    ZIO.logInfo("Streamvault pipeline starting") *>
      (MediaUploadedConsumer.consume <&> WatchSyncConsumer.consume <&> Server.serve(HealthServer.routes)).unit
        .provide(
          AppConfig.layer,
          FileDownloader.live,
          MediaUploadedConsumer.live,
          WatchSyncConsumer.live,
